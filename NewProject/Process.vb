...
Public Class Process
    'Private Connection As SqlConnection = New SqlConnection("Server=Jerry;Database=other;User Id=sa;Password=569874123;") 'GEOFF\FRANKSQL
    ...
    Public Sub New(ProcessConfig As ProcessSetup, LogDelegate As MainForm.LogDelegate)
        ...
        Me.Connection = New SqlConnection(Config.RDM_CnStr)
        Me.Connection.Open()
    ...
    Private Function ValidateFiles() As Boolean
        ...
        For Each File In BarcodeList
            'logical problem - there will never be a File without a FileStart as the first page 
            ...
            If Not File.Last.SectionName = "FE" Then
                Exceptions.Add("File end missing in file " + File.First().BarcodeValue + " - Original Page: " + File.First().OrigPageNo)
                ValidateFiles = False
            End If

            ' Two or more consecutive File Ends
            If File.Where(Function(p) p.SectionName = "FE").Count > 1 Then
                Exceptions.Add("There are 2 File Ends in file " + File.First().BarcodeValue + " - Original Page: " + File.First().OrigPageNo)
                ValidateFiles = False
            End If

            ' Checks Default Section - not in use
            'If File.Where(Function(p) p.BarcodeValue = "DEFAULT").Count = 1 Then
            '    Exceptions.Add("Default Section " + File.First().BarcodeValue)
            '    ValidateFiles = False
            'End If
            ...
            For Each requiredSection In Config.Sections.Where(Function(p) p.Required = True)
                If File.Where(Function(p) p.SectionName = requiredSection.Name).Count <= 0 Then
                    Exceptions.Add("Required section " + requiredSection.Name + " not found in file " + File.First().BarcodeValue + " - Original Page: " + File.First().OrigPageNo)
                    ValidateFiles = False
                End If
            Next
            ...
            For Each oncePerFileSection In Config.Sections.Where(Function(p) p.OncePerFile = True)
                If File.Where(Function(p) p.SectionName = oncePerFileSection.Name).Count > 1 Then
                    Exceptions.Add("Section " + oncePerFileSection.Name + " appear more than once in file " + File.First().BarcodeValue + " - Original Page: " + File.First().OrigPageNo)
                    ValidateFiles = False
                End If
            Next
            ...
            For Each Placeholder In File.Where(Function(p) p.SectionName = "PH")
                If Not Directory.Exists(LFDir.FullName + "\" + Placeholder.BarcodeValue) Then
                    Exceptions.Add("Drawing " + Placeholder.BarcodeValue + " not found " + " - Original Page: " + Placeholder.OrigPageNo)
                    ValidateFiles = False
                End If
            Next
        Next
    ...
    Private Sub ReadBarcodes()
        ...
        BarcodeReader.Options.PossibleFormats = {BarcodeFormat.CODE_39} ', BarcodeFormat.CODE_128, BarcodeFormat.CODABAR}
                ...
                If Not IsNothing(result) Then
                    ...
                    For Each section In Config.Sections
                        If section.Regex.Match(result.Text).Success Then
                            ...
                            'it's doing the job of removing the empty default sections, and we have now modified the CreateOutput() subroutine so the file starts is always added regardless

                            If (BarcodeList.Last.Last.BarcodeValue = "DEFAULT" And BarcodeList.Last.Last.Index = Index) Then
    ...
    Private Sub ReadBarcodes_Softek()
        'Dim BarcodeReader As BarcodeReader = New BarcodeReader()

        'Dim reader As New BarcodeReader
        Dim softek_reader As SoftekBarcodeNet.BarcodeReader = New SoftekBarcodeNet.BarcodeReader()
        ...
        Using softek_reader
            ...
            ' Example:
            softek_reader.LicenseKey = "7435A187026B1CE94069F7EE92ACB231"
            ...
            softek_reader.ReadCode128 = False
            softek_reader.ReadCode39 = True

            softek_reader.DatabarOptions = 255
            softek_reader.MultipleRead = False
            ' softek_reader.NoiseReduction = 0
            softek_reader.QuietZoneSize = 0
            softek_reader.LineJump = 1
            softek_reader.ScanDirection = 15
            softek_reader.SkewTolerance = 0
            softek_reader.ColorProcessingLevel = 2
            softek_reader.MinLength = 4
            softek_reader.MaxLength = 9
            softek_reader.MedianFilter = False
            softek_reader.PdfImageOnly = True
            softek_reader.PdfImageExtractOptions = 15
            softek_reader.PdfImageRasterOptions = 0
            softek_reader.PdfDpi = 300
            softek_reader.PdfBpp = 8
            ...
            For Index = 0 To ListOfImages.Count - 1
                ...
                Using tempBitmap As Bitmap = Bitmap.FromFile(ListOfImages.ElementAt(Index).FullName)
                    ...
                    'result = BarcodeReader.Decode(tempBitmap)
                    Dim nBarCodes As Integer = softek_reader.ScanBarCodeFromBitmap(tempBitmap)



                    If nBarCodes <> 0 Then
                        result = softek_reader.GetBarString(1)
                    End If



                    If Not IsNothing(result) Then
                        If Config.FStartRegex.Match(result).Success Then
                            BarcodeList.Add(New List(Of Barcode))
                            BarcodeList.Last.Add(New Barcode(Index, _
                                                             "FS", _
                                                             result, _
                        ...
                        If New Regex("^([S][P]|[S][D])(\d{6})$").Match(result).Success Then
                            BarcodeList.Last.Add(New Barcode(Index, _
                                                             "PH", _
                                                             result.Replace("SP", "SD")))
                            Continue For
                        End If

                        If New Regex("^([E][D][F][E])$").Match(result).Success Then
                            BarcodeList.Last.Add(New Barcode(Index, _
                                                             "FE", _
                                                             result, _
                                                             ListOfImages(Index).Name))
                        ...
                        For Each section In Config.Sections
                            If section.Regex.Match(result).Success Then

                                'it's doing the job of removing the empty default sections, the problem is we lose the file start sheet
                                'pendant to solve
                                ...
                                'Raises an exception when the Default section is empty - it doesnt work
                                'If (BarcodeList.Last.Last.BarcodeValue = "DEFAULT" And BarcodeList.Last.Last.Index = Index) Then
                                '    Validate2 = False
                                '    Exceptions.Add("Default Section is empty: " + BarcodeList.Last.First().BarcodeValue)
                                'End If
    ...
    Private Sub CreateOutput()
        ...
        Dim SectionOrder As Integer = Nothing
                ...
                For Index = File.First().Index + 1 To File.Last().Index - 1
                    ...
                    If File.Exists(Function(p) p.Index = localIndex) Then
                        ...
                        Else
                            ...
                            ' Removing the conditional IF segment, so the file start always get added to the file, regardless of position and section
                            ' Also check when there is no OCR


                            If (SectionOrder = 0) And (SectionIndex = 1) Then
                ...
                For Index = File.First().Index + 1 To File.Last().Index - 1
                    ...
                    If File.Exists(Function(p) p.Index = localIndex) Then
                        ...
                        Else
                            ...
                            'Removing the "IF" segment, will add always the file start to all of the sections of the file, regardless of position, etc

                            ' The new counter SectionOrder allow me to insert the file start in the first section scanned (printed section or default)
                            'Remember you also need to modify this for when the output uses OCR
